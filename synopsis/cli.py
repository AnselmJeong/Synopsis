#!/usr/bin/env python3
"""
Synopsis CLI - EPUB 분석 및 검색 시스템 통합 실행 스크립트
"""
...
from .core.search import SearchSystem, SearchType
...
def process_cmd():
    """EPUB 파일 처리 명령 (독립 실행용)"""
    parser = argparse.ArgumentParser(description="EPUB 파일 처리")
...
def search_cmd():
    """검색 명령 (독립 실행용)"""
    parser = argparse.ArgumentParser(description="콘텐츠 검색")
...
예제:
  ...
  # EPUB 파일 처리 (계층적 청킹)
  synopsis process book.epub --target-chunk-size 4096
  
  # 하이브리드 검색
  synopsis search "인공지능" --search-type hybrid
